...
best.locale <- function(sites,
  ...
  for (j in 1:iterations) {
    ...
    for(i in 1:n){
      ...
      colonize <- ifelse(sum(e.mat[-nrow(connect.mat2),nrow(connect.mat2)])>1,1,0)
    ...
    for (j in 1:iterations) {
      ...
      for(i in 1:n){
        ...
        if(!is.null(met.size)){
          ...
          philo.emig <- round(pop.size) %>%
            mutate(survive = rbinom(n(),round(pop.size$pop.size),p.survive$p.survive),
                   philo=rbinom(n(),survive,p.philo),
                   emigrant = survive-philo)
        } else {
          philo.emig <- round(pop.size) %>%
            mutate(survive = rbinom(n(),round(pop.size$pop.size),p.survive),
                   philo=rbinom(n(),survive,p.philo),
                   emigrant = survive-philo)
        }
        ...
        colonize <- ifelse(colSums(e.mat)[restore.sites[i]]>1,1,0)
